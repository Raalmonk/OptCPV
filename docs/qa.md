...
For every generated built-in artifact:
...
All built-ins must have no component overlaps and no `wire_crosses_component_body` violations.

## Known Visual Issues

Instrumentation amplifier remains fatal-free but has one known warning:

- `wire_crossing` between `N_GAIN_TOP` and `VINP`

This is tracked explicitly in `test_quality_gates.py`. The preferred fix is a cleaner in-amp gain/input bus route or a formal jumper convention, not relaxing the critic.
...
The viewer embeds artifact JSON inline, injects `artifact.svg`, creates zoom/focus buttons with DOM event listeners, and highlights components/nets/labels by SVG data attributes. It intentionally avoids inline `onclick` strings so unusual ids cannot break JavaScript.

## Adding New Circuit Cases

1. Add a fixture builder in `backend/app/schem_forge/examples.py`.
...
5. Inspect `debug.html` and `qa_summary.json`.
...
- manual inspection of generated `debug.html`
